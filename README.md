# USS Zupp1 Operations Manual

This repository is the canonical institutional archive for Zupp1 philosophy, operating procedures, Captain's logs, engineering standards, customer-experience doctrine, bridge roles, research summaries, and cultural memory.

**Status:** Founding Edition  
**Canonical archive:** Yes

## Index

### Operations Manual

- `Operations-Manual/01-Mission.md`
- `Operations-Manual/02-Values.md`
- `Operations-Manual/03-Chain-of-Command.md`
- `Operations-Manual/04-Standing-Orders.md`
- `Operations-Manual/05-Mission-Status.md`
- `Operations-Manual/06-Decision-Doctrine.md`
- `Operations-Manual/07-Research-Protocol.md`
- `Operations-Manual/08-Communications-Protocol.md`
- `Operations-Manual/09-Customer-Service-Doctrine.md`
- `Operations-Manual/10-Documentation-Retention-Protocol.md`
- `Operations-Manual/11-SO-004-Knowledge-Preservation-and-Information-Lifecycle.md`

### Engineering

- `Engineering/Hero-Video-Standard.md`
- `Engineering/Card-Alignment-Standard.md`
- `Engineering/WordPress-Editing-Standards.md`

### Customer Experience

- `Customer-Experience/Central-de-Atendimentos.md`

### Research

- `Research/MQ-UX-04-Archive-Summary.md`
- `Research/MQ-UX-04-Full-Strategic-Assessment.md`
- `Research/Intelligence/INT-001/INT-001-The-State-of-Home-Decision-Making-in-Brazil.md`
- `Research/Intelligence/INT-001/Source-Notes/Consumer-Behavior.md`
- `Research/Intelligence/INT-001/Source-Notes/Marketplaces.md`
- `Research/Intelligence/INT-001/Source-Notes/Gaps-and-Regulation.md`
- `Research/Meta-Intelligence/MQ-META-01-Audience-Segmentation.md`
- `Research/Strategy/Zupp1-Hypothesis-Measurement-Framework.md`
- `Research/Mission-Queue/Mission-Queue.md`

### Bridge Logs

- `Bridge-Logs/Ro/README.md`
- `Bridge-Logs/Ro/2026/2026-08-11-documentation-retention.md`
- `Bridge-Logs/Scotty/README.md`
- `Bridge-Logs/Spock/README.md`

### Captain's Logs

- `Captain-Logs/2026/2026-08-11.md`

### Philosophy

- `Philosophy/Founding-Principles.md`

### Poetry

- `Poetry/John/Where-No-Man-Has-Been-Before.md`

## Archival rule

Significant events, standards, decisions, mission closures, lessons learned, and durable cultural principles should be recorded automatically by the Shipmaster and Log Keeper without requiring case-by-case approval.

Do not archive passwords, secrets, financial credentials, unnecessary personal data, or unverified claims presented as fact.

## Core doctrine

> Toda boa escolha começa com compreensão.

> Todo bom atendimento começa ouvindo.
